...
class TestGeminiAgentIntegrationFiles:
    ...
    def test_real_query_with_file(self, agent, temp_python_file):
        ...
        result = agent.query_with_file(
            "List the functions in this Python file",
            temp_python_file
        )
    ...
    def test_real_code_review(self, agent, temp_python_file):
        ...
        assert any(word in response_text for word in [
            "code", "function", "review", "quality", "good", "clean"
        ])
    ...
    def test_real_generate_docs(self, agent, temp_python_file):
        ...
        assert any(word in response_text for word in [
            "add", "multiply", "function", "parameter", "return"
        ])
...
""")
        ...
        assert any(word in response_text for word in [
            "error", "connection", "timeout", "issue", "problem"
        ])
...
""")
        ...
        results = agent.batch_process(
            str(tmp_path),
            "List the function name in this file",
            file_pattern="*.py"
        )
...
class TestGeminiAgentIntegrationEndToEnd:
    ...
    def test_complete_workflow(self, agent, temp_buggy_file):
        ...
        has_issues = any(word in review_text for word in [
            "zero", "division", "error", "check", "validation", "bug"
        ])
