...
class TestClaudeAgentIntegrationBasic:
    ...
    def test_real_query(self, agent):
        ...
        result = agent.query("Say 'Claude integration test successful' and nothing else")
        
        assert result is not None
        assert "result" in result
        
        response_text = result["result"].lower()
        assert "claude" in response_text or "integration" in response_text or "successful" in response_text
...
class TestClaudeAgentIntegrationFiles:
    ...
    def test_real_query_with_stdin(self, agent, temp_python_file):
        ...
        result = agent.query_with_stdin(
            "List all function names in this Python code",
            content
        )
    ...
    def test_real_code_review(self, agent, temp_buggy_file):
        ...
        assert any(word in response_text for word in [
            "error", "exception", "check", "validation", "bug", "issue", "unsafe"
        ])
    ...
    def test_real_generate_docs(self, agent, temp_python_file):
        ...
        assert any(word in response_text for word in [
            "calculate", "function", "parameter", "return", "sum", "average"
        ])
    ...
    def test_real_fix_code(self, agent, temp_buggy_file):
        ...
        result = agent.fix_code(
            temp_buggy_file,
            "Add proper error handling for zero division"
        )
        ...
        assert any(word in response_text for word in [
            "fix", "error", "exception", "try", "catch", "handle"
        ])
...
""")
        ...
        results = agent.batch_process(
            str(tmp_path),
            "List the function names in this file",
            file_pattern="*.py"
        )
...
Be concise."""
        ...
        security_keywords = ["zero", "division", "error", "exception", "check", "validation", "unsafe"]
        has_security_issue = any(keyword in response_text for keyword in security_keywords)
        ...
        has_function = any(func.replace("_", "") in response_text.replace("_", "") 
        ...
        suggestion_keywords = ["add", "try", "catch", "if", "check", "validate", "handle"]
        has_suggestion = any(keyword in response_text for keyword in suggestion_keywords)
        ...
        if "session_id" in result:
            ...
            followup = agent.continue_conversation(
                "Which function is most critical to fix first?",
                session_id=session_id
            )
            ...
            references_function = any(func.replace("_", "") in followup_text.replace("_", "")
                                     for func in function_names)
...
class TestClaudeAgentIntegrationPerformance:
    ...
    def test_large_file_handling(self, agent, tmp_path):
        ...
        has_count = any(indicator in response_text.lower() for indicator in count_indicators)
        
        # More lenient check - just verify it processed the file
        assert len(response_text) > 0, (
            f"Should provide a response for large file. "
            f"Got: {response_text}"
        )
